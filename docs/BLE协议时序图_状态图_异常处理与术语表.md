...
> 发送的同时，GyroBleManager 启动 2s 应答看门狗：超时自动重发（默认最多 2 次），
> 重试耗尽回调 `onCommandTimeout(0x01)`；收到匹配 ACK 即停止定时器（见异常流 F.3）。
...
> 两级看门狗：阶段①发送时启动 2s 应答看门狗（超时重发，耗尽回调 `onCommandTimeout`）；
> 阶段②寻北被受理后启动 120s 寻北结果看门狗等待 0x06，超时回调 `onNorthSeekTimeout`，
> 阶段③收到 0x06 即取消该看门狗（见异常流 F.3 / F.4）。
...
> 阶段①发送 0x03 时同步启动 2s 应答看门狗：超时自动重发，重试耗尽回调
> `onCommandTimeout(0x03)`（见异常流 F.3）；阶段③的连续数据帧无应答机制，
> 依靠帧序号检测丢帧（见异常流 F.6）。
