...
**字段边界逐字节标注**（LEN 域只计 DATA 的字节数，不含帧头、CMD、CHK）：

| 字段 | 帧头 | LEN | CMD | DATA（4 字节） | CHK |
|---|---|---|---|---|---|
| 设置纬度 | `AA 55` | `04` | `01` | `02 60 E3 C8`（39904200，int32 大端） | `12` |

| 字段 | 帧头 | LEN | CMD | DATA（8 字节） | CHK |
|---|---|---|---|---|---|
| 导航数据 | `AA 55` | `08` | `04` | `30 39` 航向 + `FE 0C` 俯仰 + `00 FA` 横滚 + `03` 状态 + `07` 序号 | `83` |

> 各示例长度域核对：设置纬度 DATA=4 字节→LEN=04；导航数据 DATA=8 字节→LEN=08；
> 寻北/进入导航/退出导航无 DATA→LEN=00；通用应答 DATA=2 字节→LEN=02；寻北结果 DATA=3 字节→LEN=03。
> 与代码 `GyroProtocol.buildFrame()` 中 `frame[2] = (byte) data.length` 完全一致，整帧长度 = 5 + LEN。
